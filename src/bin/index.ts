...
program
  .name("jz-deploy")
  .description(
    "JZ-Deploy 是一款由简知前端自研，致力于让简知前端应用 CI/CD 流程更加简单高效的 CLI 工具~"
  )
  .version(version, "-v, --version", "输出版本信息")
  ...
  .addHelpText(
    ...
    `\n运行 ${cyan(
      "jz-deploy <command> -h"
    )} 可查看指定命令帮助信息，或查看在线文档 https://jz-tech.yuque.com/jz-tech/fe-cicd`
  );
...
program
  ...
  .action((...params) => {
    ...
    if (argsLength === 0) {
      warn("请指定初始化环境，可通过 -h 查看帮助");
      return;
    }
    if (argsLength > 1) {
      warn("不允许同时初始化服务端和项目端配置");
      return;
    }
